...
build:
	cd result && docker build --target nodejs -t kahiki.jfrog.io/kahiki-stable/result-nodejs:${GIT_SHA} --build-arg GIT_SHA=$(GIT_REV) --file Dockerfile . && docker push kahiki.jfrog.io/kahiki-stable/result-nodejs:${GIT_SHA} && docker build --target nginx -t kahiki.jfrog.io/kahiki-stable/result-nginx${GIT_SHA} --build-arg GIT_SHA=$(GIT_REV) --file Dockerfile . && docker push kahiki.jfrog.io/kahiki-stable/result-nginx:${GIT_SHA}
